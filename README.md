...
A full-stack productivity and task-management web app with a Notion-style task planner, a per-day checklist, live time-progress trackers, and XP/streak gamification, backed by a multi-user cloud database.

Dunzo started as a single-user localStorage prototype but was migrated to a database synced application with a typed backend, real authentication, and per-user data scoping.
...
- requireAuth - verifies the Neon Auth JWT against JWKS and sets req.userId; every route is scoped by user_id
